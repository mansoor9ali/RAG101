...
| Technique | LangChain | LlamaIndex | Runnable Script | Overview | Implementation | Additional Resources |
| :--- | :---: | :---: | :---: | :--- | :--- | :--- |
| **Simple RAG 🌱** | ✅ | ✅ | ✅ | Basic RAG techniques for newcomers. | Start with basic retrieval queries and integrate incremental learning mechanisms. | |
| **Simple RAG using a CSV file 🧩** | ✅ | ✅ | | Basic RAG using CSV files, integrated with OpenAI for Q\&A. | Uses CSV files to create basic retrieval and integrates with OpenAI to create a question and answering system. | |
| **Reliable RAG 🏷️** | | | | Enhances Simple RAG by adding validation and refinement for accuracy. | Checks for retrieved document relevancy and highlights the segment of documents used for answering. | |
| **Choose Chunk Size 📏** | ✅ | | ✅ | Selecting an appropriate fixed size for text chunks. | Experiment with different chunk sizes to find the optimal balance between context and speed. | |
| **Proposition Chunking ⛓️‍💥** | | | ✅ | Breaks down text into concise, complete, meaningful sentences for better knowledge extraction. | **Proposition Generation:** Uses an LLM with a custom prompt to generate factual statements from document chunks. \<br\> **Quality Checking:** Passes generated propositions through a grading system for accuracy, clarity, completeness, and conciseness. | The Propositions Method: Enhancing Information Retrieval for AI Systems |
...
| Technique | LangChain | LlamaIndex | Runnable Script | Overview | Implementation | Additional Resources |
| :--- | :---: | :---: | :---: | :--- | :--- | :--- |
| **Query Transformations 🔄** | ✅ | | ✅ | Modifying and expanding queries to improve retrieval effectiveness. | **Query Rewriting:** Reformulate queries to improve retrieval. \<br\> **Step-back Prompting:** Generate broader queries for better context retrieval. \<br\> **Sub-query Decomposition:** Break complex queries into simpler sub-queries. | |
| **Hypothetical Questions (HyDE Approach) ❓** | ✅ | | ✅ | Generating hypothetical questions to improve alignment between queries and data. | Creates hypothetical questions that point to relevant locations in the data, enhancing query-data matching. | HyDE: Exploring Hypothetical Document Embeddings for AI Retrieval |
...
| Technique | LangChain | LlamaIndex | Runnable Script | Overview | Implementation | Additional Resources |
| :--- | :---: | :---: | :---: | :--- | :--- | :--- |
| **Hypothetical Prompt Embeddings (HyPE) ❓🚀** | ✅ | | ✅ | Precomputes hypothetical prompts at the indexing stage, transforming retrieval into a question-question matching task. | **Precomputed Questions:** Generates multiple hypothetical queries per chunk at indexing time. \<br\> **Question-Question Matching:** User queries are matched against stored hypothetical questions. \<br\> **No Runtime Overhead:** Does not require LLM calls at query time. \<br\> **Higher Precision & Recall:** Improves retrieval context precision and claim recall. | Preprint: Hypothetical Prompt Embeddings (HyPE) |
| **Contextual Chunk Headers 🏷️** | | | | Creates document-level and section-level context, prepending headers to chunks. | Creates a chunk header that includes context about the document and/or section, and prepends it to each chunk to improve retrieval accuracy. | dsRAG: open-source retrieval engine that implements this technique |
| **Relevant Segment Extraction 🧩** | | | | Dynamically constructs multi-chunk text segments relevant to a given query. | Performs a retrieval post-processing step to analyze relevant chunks and identify longer, multi-chunk segments for more complete context. | |
| **Context Enrichment Techniques 📝** | ✅ | ✅ | ✅ | Enhances retrieval accuracy by embedding individual sentences and extending context to neighboring sentences. | Retrieves the most relevant sentence while also accessing the sentences before and after it in the original text. | |
| **Semantic Chunking 🧠** | ✅ | | ✅ | Divides documents based on semantic coherence rather than fixed sizes. | Uses NLP techniques to identify topic boundaries or coherent sections for more meaningful retrieval units. | Semantic Chunking: Improving AI Information Retrieval |
| **Contextual Compression 🗜️** | ✅ | | ✅ | Compresses retrieved information while preserving query-relevant content. | Uses an LLM to compress or summarize retrieved chunks, preserving key information relevant to the query. | |
