...
class WeatherService:
    """
    Weather query service based on Amap (Gaode) API
    """
    ...
    def get_city_code(self, city_name: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        Returns:
            Tuple[Optional[str], Optional[str]]: (adcode, city name) tuple; (None, None) if not found
        """
        try:
            params = {
                "key": self.api_key,
            ...
            if data["status"] == "1" and data["count"] != "0":
                ...
                logger.warning(f"City not found: {city_name}, API response: {data}")
                return None, None
    ...
    def query_weather(self, city: str, extensions: str = "all") -> Dict[str, Any]:
        ...
        Args:
            ...
            extensions: Weather type, base=current weather, all=3-day forecast
        ...
        try:
            ...
            if data["status"] == "1":
                ...
                result["message"] = f"Weather query failed, API response: {data}"
                
            return result
            
        except Exception as e:
            logger.error(f"Error querying weather: {str(e)}")
            result["message"] = f"Error querying weather: {str(e)}"
            return result
    ...
    def _format_current_weather(self, weather: Dict[str, Any], city_name: str) -> str:
        return (
            f"Current weather in {city_name}: {weather.get('weather')}, temperature {weather.get('temperature')}℃, "
            f"humidity {weather.get('humidity')}%, {weather.get('winddirection')} wind level {weather.get('windpower')}. "
            f"Report time: {weather.get('reporttime')}"
        )
    ...
    def _format_forecast_weather(self, forecast: Dict[str, Any], city_name: str) -> str:
        ...
        for cast in forecast.get("casts", []):
            ...
            result += (
                f"{date}: Day {day_weather} {day_temp}℃ {day_wind}, "
                f"Night {night_weather} {night_temp}℃ {night_wind}\n"
            )
...
class WeatherTools:
    """
    Weather query tools based on Amap (Gaode) API
    """
